...
DEFAULT_TIMEOUT = 30
POLL_INITIAL_DELAY = 2
POLL_MAX_DELAY = 60
POLL_TOTAL_TIMEOUT = 15 * 60
...
ATTACHMENTS_ROOT = Path(settings.BASE_DIR) / "attachments" / "reports"
...
def wait_until_processed(report_id: str, *, timeout: int = POLL_TOTAL_TIMEOUT) -> Dict[str, Any]:
...
def _build_filename(report_type: str) -> str:
...
def build_report_filename(report_type: str) -> str:
    """Retorna un nombre de archivo basado en el tipo de reporte."""
    return _build_filename(report_type)
